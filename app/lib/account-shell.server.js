...
export function accountShell({
  ...
  <div class="hyve-acct__contact">
    ${contactPanel()}
    <a class="hyve-acct__whatsapp" href="${WHATSAPP_URL}" target="_blank" rel="noopener" aria-label="Chat on WhatsApp">
      ${icoWhatsApp()}
    </a>
  </div>
  ${CONTACT_SCRIPT}
...
/* ---------- icons ---------- */
function svg(inner) {
  return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" width="14" height="14">${inner}</svg>`;
}
...
/**
 * HYV-97: both channels and the one set of hours, on every page. A native
 * disclosure, so it opens without any script; the script below only lets the
 * dashboard's Contact Support tile open it too, and closes it on Escape or a
 * click elsewhere.
 */
function contactPanel() {
  return `
    <details class="hyve-acct__contact-panel" id="hyve-contact">
      <summary class="hyve-acct__contact-btn" aria-label="Contact customer service">${icoMail()}</summary>
      <div class="hyve-acct__contact-card">
        <strong class="hyve-acct__contact-title">Contact customer service</strong>
        <a class="hyve-acct__contact-row" href="${WHATSAPP_URL}" target="_blank" rel="noopener">
          <span class="hyve-acct__contact-icon hyve-acct__contact-icon--wa">${icoWhatsApp()}</span>
          <span><span class="hyve-acct__contact-label">WhatsApp</span><span class="hyve-acct__contact-value">${WHATSAPP_DISPLAY}</span></span>
        </a>
        <a class="hyve-acct__contact-row" href="mailto:${SUPPORT_EMAIL}">
          <span class="hyve-acct__contact-icon">${icoMail()}</span>
          <span><span class="hyve-acct__contact-label">Email</span><span class="hyve-acct__contact-value">${SUPPORT_EMAIL}</span></span>
        </a>
        <p class="hyve-acct__contact-hours">${SERVICE_HOURS}</p>
      </div>
    </details>`;
}
...
<script>
(function () {
  var panel = document.getElementById('hyve-contact');
  if (!panel) return;
  document.addEventListener('click', function (event) {
    var opener = event.target.closest && event.target.closest('a[href="#hyve-contact"]');
    if (opener) {
      event.preventDefault();
      panel.open = true;
      var first = panel.querySelector('.hyve-acct__contact-row');
...
<style>
  ...
  /* Fixed outside .hyve-acct, so it carries the portal's tokens and type itself. */
  .hyve-acct__contact {
    ${PORTAL_TOKENS}
    position: fixed; right: 20px; bottom: 20px; z-index: 40; display: flex; flex-direction: column; align-items: center; gap: 10px;
    font-family: var(--hyve-body); font-size: 14px; color: var(--hyve-900);
  }
  ...
  .hyve-acct__contact-panel { position: relative; }
  .hyve-acct__contact-panel > summary { list-style: none; cursor: pointer; }
  .hyve-acct__contact-panel > summary::-webkit-details-marker { display: none; }
  .hyve-acct__contact-card {
    position: absolute; right: calc(100% + 12px); bottom: 0; width: 264px;
    background: var(--hyve-white); border: 1px solid var(--hyve-border); border-radius: 14px;
    box-shadow: 0 20px 25px -5px rgba(15,23,42,0.12); padding: 14px;
    display: flex; flex-direction: column; gap: 8px;
  }
  .hyve-acct__contact-title { font-size: 14px; font-weight: 700; color: var(--hyve-900); }
  .hyve-acct__contact-row {
    display: flex; align-items: center; gap: 10px; padding: 8px; border-radius: 10px;
    text-decoration: none; color: var(--hyve-900);
  }
  .hyve-acct__contact-row:hover, .hyve-acct__contact-row:focus-visible { background: #F1F5F9; }
  .hyve-acct__contact-icon {
    width: 34px; height: 34px; border-radius: 9999px; flex: none; display: inline-flex;
    align-items: center; justify-content: center; background: #F1F5F9; color: var(--hyve-700);
  }
  .hyve-acct__contact-icon svg { width: 18px; height: 18px; }
  .hyve-acct__contact-icon--wa { background: #25D366; color: #fff; }
  .hyve-acct__contact-label { display: block; font-size: 12px; color: var(--hyve-muted); }
  .hyve-acct__contact-value { display: block; font-size: 14px; font-weight: 600; }
  .hyve-acct__contact-hours { margin: 2px 0 0; font-size: 12px; color: var(--hyve-muted); }
